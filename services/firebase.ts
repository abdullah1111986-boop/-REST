...
import { FirebaseConfig, Subject, Trainee } from '../types';
...
export const searchTraineeByNationalId = async (nationalId: string): Promise<Trainee | null> => {
  ...
  // Firestore doesn't support logical OR in one query easily without composite indexes, 
  // so we'll search by nationalId first, if not found, search by traineeNumber manually or separate query.
  
  // Try National ID
  const q1 = query(collection(db, 'trainees'), where("nationalId", "==", nationalId));
  ...
  // Try Trainee Number (if the input might be one)
  const q2 = query(collection(db, 'trainees'), where("traineeNumber", "==", nationalId));
...
export const processBulkImport = async (data: ImportedData) => {
  ...
  for (const [nationalId, tData] of data.trainees) {
    ...
    if (nationalIdToDocId.has(nationalId)) {
      ...
      const traineeRef = doc(db, 'trainees', docId);
      batch.update(traineeRef, finalTraineeData);
    } else {
      // Create new
      const newDocRef = doc(collection(db, 'trainees'));
